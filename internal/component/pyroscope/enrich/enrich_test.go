...
func TestEnricher(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Create a test appendable to capture the enriched profile
			...
			// Process a profile entry through the Append method
			ctx := context.Background()
			err = comp.exports.Receiver.Appender().Append(ctx, tt.inputLabels, testSamples)
			require.NoError(t, err)

			// Wait for the profile to be received
			wg.Wait()
